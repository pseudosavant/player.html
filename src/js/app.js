    ...
    const seek = (delta) => {
      ...
      if (!duration) {
        ...
        $player.currentTime = minmax(0, newTime, duration);
      }
    ...
    const setMediaSessionActionHandler = (action, handler) => {
      ...
      try {
        ...
        if (isDebug()) console.warn(`Media Session action not supported: ${action}`, e);
      }
    }

    const getCurrentPlaylistItem = () => {
      const currentIndex = getPlaylistCurrentIndex();
      if (currentIndex < 0 || !Array.isArray(app.playlist)) return null;
      return app.playlist[currentIndex] || null;
    }
    ...
    const getMediaSessionAlbum = () => {
      ...
      try {
        ...
        const parts = folder.pathname.split('/').filter((part) => part.length > 0);
        const folderName = parts.length > 0 ? parts[parts.length - 1] : folder.host;
    ...
    const cssUrlToMediaImageUrl = (value) => {
      const match = String(value || '').trim().match(/^url\((["']?)(.*)\1\)$/);
      return (match && match[2] ? match[2] : '');
    }

    const getMediaImageType = (url) => {
      const clean = String(url || '').split('?')[0].split('#')[0].toLowerCase();
      if (clean.startsWith('data:image/')) {
        const match = clean.match(/^data:(image\/[^;,]+)/);
    ...
    const getMediaSessionArtwork = () => {
      ...
      const candidates = [tileArtworkUrl, fallbackIcon ? fallbackIcon.href : ''].filter((url) => url && url.length > 0);

      return candidates.map((url) => {
        const type = getMediaImageType(url);
        const image = { src: url };
    ...
    const updateMediaSessionMetadata = () => {
      if (!mediaSessionAvailable() || typeof MediaMetadata !== 'function') return;

      try {
        navigator.mediaSession.metadata = new MediaMetadata({
          title: getMediaSessionTitle(),
          artist: getMediaSessionAlbum(),
          album: document.title,
          artwork: getMediaSessionArtwork()
        });
      } catch (e) {
        if (isDebug()) console.warn('Unable to update Media Session metadata', e);
      }
    }
    ...
    const updateMediaSessionPlaybackState = (state) => {
      if (!mediaSessionAvailable()) return;
      try {
        navigator.mediaSession.playbackState = state === 'play' ? 'playing' : (state === 'pause' ? 'paused' : 'none');
      } catch (e) {
        if (isDebug()) console.warn('Unable to update Media Session playback state', e);
      }
    }
    ...
    const updateMediaSessionPositionState = () => {
      ...
      const playbackRate = Number.isFinite($player.playbackRate) && $player.playbackRate > 0 ? $player.playbackRate : 1;
      ...
      try {
        ...
        navigator.mediaSession.setPositionState({
          ...
          position: minmax(0, position, duration)
        });
      } catch (e) {
        if (isDebug()) console.warn('Unable to update Media Session position state', e);
      }
    }
    ...
    const resetMediaSession = () => {
      ...
      try {
        ...
        if (isDebug()) console.warn('Unable to reset Media Session state', e);
      }
    }
    ...
    const setupMediaSession = () => {
      ...
      setMediaSessionActionHandler('stop', actionStop);
      setMediaSessionActionHandler('seekbackward', () => {
        if (!$player.src) return;
        actionReplay();
      });
      setMediaSessionActionHandler('seekforward', () => {
        if (!$player.src) return;
        actionSkip();
      });
      setMediaSessionActionHandler('seekto', (details = {}) => {
        if (!$player.src || !Number.isFinite(details.seekTime)) return;
        const seekTime = minmax(0, details.seekTime, getMediaDuration());
    ...
    const updateMediaSessionActionHandlers = () => {
      if (!mediaSessionAvailable()) return;
      if (hasMultiPlaylist()) {
    ...
    const setupPlayerEvents = () => {
      ...
      $player.on('ratechange', updateMediaSessionPositionState);
    ...
    const getSVGPoster = (colorValue) => {
      ...
      const svg = `<?xml version='1.0' encoding='UTF-8' standalone='no'?>
        <svg width='37.101268' height='38.292346' viewBox='5.908228 4.854162 37.101268 38.292346' version='1.1' xmlns='http://www.w3.org/2000/svg'>
