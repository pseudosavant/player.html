...
* Support for playing videos directly from OneDrive. You must supply the appropriate keys in the `app.options.cloud` at AND register your app with Microsoft. `player.html` also *must be served over HTTPS* for the auth flows to work. [Remix this Glitch](https://glitch.com/edit/#!/player-html-remix?path=src%2Fplayer.html%3A487%3A10) to easily check it out over HTTPS with your own API keys.
